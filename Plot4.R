...
# I finally registered my plot image with the appropriate button in Rstudio and set the image height manually with this option

#code for making plot 2
#Starting from the code employed for plot 1 - see above for details
#EXTRA NOTE: Consider that my Rstudio is in french, so on the X axis, jeu. stand for Thur, vend. for Fri and sam. for Sat
...
par(mfrow = c(2,2))
par(mar = c(5,5,1,1))
plot(datattime, GAP, type="l", ylab="Global Active Power", xlab="")
volt<-data_last[["Voltage"]]
plot(datattime,volt, type="l", xlab="datetime",ylab="Voltage")
plot(datattime,subm1, type="l", xlab="",ylab="Energy sub metering")
with(lines(datattime,subm2, col="red", xlab=""))
with(lines(datattime,subm3, col="blue", xlab=""))
legend("topright",legend=c("Sub_metering_1","Sub_metering_2","Sub_metering_3"),bty = "n",lwd=1.5, cex=0.9,y.intersp=0.8, col=c("black","red","blue"), lty=c(1,1,1))
GRP<-data_last[["Global_reactive_power"]]
plot(datattime,GRP, type="l", xlab="datetime",ylab="Global_reactive_power")
# I finally registered my plot image with the appropriate button in Rstudio and set the image height and width manually with this option
